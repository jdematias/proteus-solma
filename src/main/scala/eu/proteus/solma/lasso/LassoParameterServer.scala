...
object LassoParameterServer {
  ...
  def transformLasso(model: Option[DataStream[(Int, LassoModel)]] = None)
    ...
    val concreteModelBuilder = new LassoModelBuilder(initConcrete(1.0, 0.0, 0.1, featureCount)(0))

    transformGeneric[LassoParam, ((Long, Double), Double), LassoModel, OptionLabeledVector, LassoStreamEvent,
      Int](model)(initConcrete(1.0, 0.0, 0.1, featureCount), concreteModelBuilder.addParams, concreteModelBuilder
    )(
